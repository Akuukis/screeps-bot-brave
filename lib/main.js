...
function roleTrans(creep){
	if(creep.energy == 0 || (Memory.creeps[creep.name].state == "empty" && creep.energy < creep.energyCapacity) ) {
		...
		if(_.size(t)>0){
			...
			if(target){ Memory.creeps[creep.name].target=Game.spawns[target]; };
			if(!target) {
				//console.log("SEIT")
				for(creep2 in Game.creeps) {
					if(  Memory.creeps[Game.creeps[creep2].name].role == "contr"
						&& Game.creeps[creep2].energy < Game.creeps[creep2].energyCapacity * 0.8
					){
						target=creep2;
					};
				};
				if(target){ Memory.creeps[creep.name].target=Game.creeps[target]; };
			};
			if(!target){
				for(creep2 in Game.creeps){
					if(  Memory.creeps[Game.creeps[creep2].name].role == "settler"
						&& Game.creeps[creep2].energy < Game.creeps[creep2].energyCapacity * 0.8
					){
						target=creep2;
					};

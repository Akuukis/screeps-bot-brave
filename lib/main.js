...
//// 1.  //// Memory structure check, assume good later.
{
	...
	if(!Memory.signups){ Memory.signups={}; };
...
for (id in Memory.squads.mine){
	...
	if(!squad.options){
		...
		squad.options = [
			{e:0, roles:{ miner:{M:1,W:2}, slayer:{M:1,A:3}, collector:{M:1,C:5 } } },
			{e:0, roles:{ miner:{M:2,W:2}, slayer:{M:1,A:3}, collector:{M:1,C:5 } } },
			{e:1, roles:{ miner:{M:1,W:3}, slayer:{M:1,A:3}, collector:{M:2,C:5 } } },
			{e:2, roles:{ miner:{M:2,W:3}, slayer:{M:1,A:4}, collector:{M:2,C:6 } } },
			{e:3, roles:{ miner:{M:1,W:4}, slayer:{M:1,A:5}, collector:{M:2,C:7 } } },
			{e:4, roles:{ miner:{M:2,W:4}, slayer:{M:1,A:5}, collector:{M:2,C:8 } } },
			{e:5, roles:{ miner:{M:1,W:5}, slayer:{M:1,A:6}, collector:{M:2,C:9 } } },
			{e:6, roles:{ miner:{M:2,W:5}, slayer:{M:1,A:6}, collector:{M:2,C:10} } },
			{e:7, roles:{ miner:{M:3,W:5}, slayer:{M:1,A:7}, collector:{M:3,C:10} } },
			{e:8, roles:{ miner:{M:3,W:5}, slayer:{M:1,A:8}, collector:{M:3,C:11} } },
			{e:9, roles:{ miner:{M:3,W:6}, slayer:{M:1,A:8}, collector:{M:3,C:12} } }
		];
	};
	if(!squad.wish){
	...
	var exts = false;
	for(i in Memory.cities){ if(Memory.cities[i].ext>=squad.wish.e){ exts=true; }; };
	if(squad.creeps.length==0 && (!squad.uniq.lair || (gobi(id).room.controller && gobi(id).room.controller.level>1))){
		squad.state="idle";
		for(i in squad.wish.c){	
			Memory.signups[id+"-"+i]=[id, squad.wish.e, 60, Memory.taps[squad.tap].pos, squad.wish.c[i]); };
	};

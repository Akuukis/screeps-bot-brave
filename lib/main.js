...
for(name in Memory.cities){
	...
	if(room){ // If visible.
		...
		if(!city.ext && city.ext!=0){ 
			city.ext = room.find(FIND_MY_STRUCTURES, {filter: {structureType: STRUCTURE_EXTENSION}}).length;
		};
	};
};
		...
		for(i in squad.wish.creeps){
			...
			if(!Memory.demand[id+"-"+i]){
				...
				Memory.demand[id+"-"+i] = {
					...
					at: Game.time+60,
					destination: new RoomPosition(tap.pos.x,tap.pos.y,tap.pos.roomName),
					risky: false,
...
for(name in Memory.cities){
	...
	if(Math.floor(Game.time/3) == Game.time/3){
		for(name in city.spawns){
			var spawn = city.spawns[name];
			if(spawn.next && spawn.queue[spawn.next] && Memory.supply[spawn.queue[spawn.next]]){
				if(spawn.next <= (Game.time-Game.time%3)/3){
					var order = Memory.supply[spawn.queue[spawn.next]];
					if(order && order.body){
						...
						if(ok == 0){
							...
							if(!order.destination){ order.destination = new RoomPosition(spawn.pos.x, spawn.pos.y, spawn.pos.roomName); };
							if( order.risky===undefined){ order.risky=false; };
							if(!order.memory){ order.memory={}; };
							order.memory.owner = order.owner;
							var mem = {
								id: spawn.queue[spawn.next],
								owner: spawn.name,
								role: "transit",
							...
							if(spawn.createCreep(order.body, order.name, mem) == 0){ 
								...
								spawn.ready = Math.floor(Game.time/3) + order.body.length;
								Memory.supply[spawn.queue[spawn.next]].status = "spawning";
								delete spawn.queue[spawn.next];
							};
						...
						for(i in spawn.queue){ first = Math.min(first,i); };
						spawn.next = (first==Number.MAX_VALUE ? false : first);
					};
				}else{
					// Do nothing. Wait for demand.
				};
			}else{
				if(spawn.queue[spawn.next] && !Memory.supply[spawn.queue[spawn.next]]){
					delete spawn.queue[spawn.next];
					delete Memory.supply[spawn.queue[spawn.next]];
				};
				var first = Number.MAX_VALUE;
				for(i in spawn.queue){ first = Math.min(first,i); };
				spawn.next = (first==Number.MAX_VALUE ? false : first);
...
for(id in []/*Memory.demand*/){ // Distribute spawning demands to spawns.
	...
	for(c in Memory.cities){
		for(s in Memory.cities[c].spawns){
			...
			console.log(">demand>cities>spawns> ",spawn,Game.spawns[s],ram,Memory.cities[c].spawns[s]);
			var distance = spawn.pos.findPathTo(demand.destination, {ignoreCreeps: true}).length;
			if(!distance){ console.log("Bad distance"); continue; };
			var moment = spawn.ready;
			while(moment < at + length){
				while(occupied){
					var occupied = false;
					for(q=moment;q<moment+length;q++){
						if(spawn.queue[q]){
							occupied = q + Memory.supply[spawn.queue[q]].body.length
						};
					};
				...
				score.wait = -Math.max(0, at - (moment+length))*cost/1800;
				score.delay = -Math.max(0, (moment+length) - at)*demand.score;
			...
			//no .at-length-distance līdz tgd
			// Calc offset. Faster=upkeep, Slower=Owner's perf.
		};
	};
	if(best.spawn){
		best.spawn.queue[moment] = id;
...
if(!Game.spawns.Spawn1.spawning) {
	...
	for(p in pipes.safe) {
	    ...
	    if(count<2) {console.log("Create miner:",Game.spawns.Spawn1.createCreep([MOVE, WORK], "miner"+getID(), {role: "miner", target: pipes.safe[p] }));};
	...
	if(countTrans>1 && countContr<3) {console.log("Create Contr:",Game.spawns.Spawn1.createCreep([MOVE, CARRY, WORK, WORK], "contr"+getID(), {role: "contr", target: contr }));};

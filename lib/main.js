...
var dTasks = { // Define tasks.
	...
	overlayNetwork: function(roomName){
		console.log(roomName,": overlayNetwork");
		var room = Game.rooms[roomName];
		var ram = Memory.rooms[roomName];
		if(!Memory.rooms[roomName].clean){
			for(i in Game.flags){ if(Game.flags[i].room.name==roomName){ Game.flags[i].remove(); }; };
		...
		for(i in ram.sources){ if(!Memory.squads.mine[i]){ Memory.squads.mine[i]={}; }; };
		// Add controller.
		...
		for(i in Memory.rooms[roomName].contr){ if(!Memory.squads.upgr[i]){ Memory.squads.upgr[i]={}; }; };
		// Add sources + contr to POIs.
		if(!Memory.rooms[roomName].pois){
			Memory.rooms[roomName].pois = [];
			for(i in Memory.rooms[roomName].sources){ Memory.rooms[roomName].pois.push(Memory.rooms[roomName].sources[i].pos); };
			for(i in Memory.rooms[roomName].contr){ Memory.rooms[roomName].pois.push(Memory.rooms[roomName].contr[i].pos); };
		};
		...
		for(i=0;i<Memory.rooms[roomName].pois.length;i++){
			if(!Memory.rooms[roomName].pe[i]){
				...
				for(j=0;j<Memory.rooms[roomName].egroups.length;j++){
					var egroup = Memory.rooms[roomName].egroups[j];
					for(k=0;k<egroup.length;k++){ egroup[k] = new RoomPosition(egroup[k].x,egroup[k].y,egroup[k].roomName); };
			...
			if(Game.getUsedCpu()/Game.cpuLimit>0.8){ return false; };
		};
		...
		for(i=Memory.rooms[roomName].pois.length-1;i>0;i--){ 
			for(j=0;j<i;j++){
				var pos1 = new RoomPosition(Memory.rooms[roomName].pois[i].x, Memory.rooms[roomName].pois[i].y, Memory.rooms[roomName].pois[i].roomName);
				var pos2 = new RoomPosition(Memory.rooms[roomName].pois[j].x, Memory.rooms[roomName].pois[j].y, Memory.rooms[roomName].pois[j].roomName);
		...
		while(Memory.rooms[roomName].condensed < 7){ // Repeat 7 times
			...
			var action = 0;
			for(i in Memory.rooms[roomName].tiles){
			...
			var count = 0; for(i in Memory.rooms[roomName].tiles){ if(Memory.rooms[roomName].tiles[i]){ count++;}; };
		...
		for(i in Memory.rooms[roomName].taps){ room.createFlag(Memory.rooms[roomName].taps[i].x, Memory.rooms[roomName].taps[i].y, room.name+"_"+i, COLOR_BLUE); };
...
while(Game.cpuLimit>400 && Game.getUsedCpu()/Game.cpuLimit<0.8 && Memory.deffered.fn.length>0){ // TODO const.
	console.log(Memory.deffered.fn[0]);
			...
			*/
			...
			for(i in Game.creeps) {
				...
				if(Memory.creeps[creep2.name].role == "contr" || Memory.creeps[creep2.name].role == "settler"){
					target[target.length] = {
						score: 1 /*(creep.room.name == creep2.room.name ? 1 : Math.max(0, avgTrans / countTrans - 1))*/,
						target: creep2
					};
				};
			};

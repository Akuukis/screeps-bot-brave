...
function roleTrans(creep){
	if(creep.energy == 0 || (Memory.creeps[creep.name].state == "empty" && creep.energy < creep.energyCapacity) ) {
		...
		if(t){
			if(creep.pos.getRangeTo(Game.getObjectById(t.id)) > 1) {
				if(!!creep.moveTo(Game.getObjectById(t.id))){ Memory.creeps[creep.name].target={}; };
			}else{
				if(!!creep.transferEnergy(Game.getObjectById(t.id))){ Memory.creeps[creep.name].target={}; };
			};
		}else{
			var target = [];
			for(i in Game.spawns){
				if(Game.spawns[i].energy < Game.spawns[i].energyCapacity) {
					target[target.length] = {
						distance: creep.pos.getPathTo(creep2) + Game.map.findRoute(creep.room, creep2.room).length * 50,
			...
			if(target.length){
				var t = target[0];
				for(i in target){ if(t.distance < target[i].distance) { t = target; }; };
			};
			if(!target.length) {
				for(creep2 in Game.creeps) {
					var creep2 = Game.creeps[i];
					if(  (Memory.creeps[creep2.name].role == "contr" || Memory.creeps[creep2.name].role == "contr")
						&& creep2.energy < creep2.energyCapacity * 0.8
					){
					target[target.length] = {
							distance: creep.pos.getPathTo(creep2) + Game.map.findRoute(creep.room, creep2.room).length * 50,
							target: creep2
...
for(i in Game.spawns){
	...
	if(!spawn.spawning) {
		...
		if(countMiners>1 && countTrans<7 && countTransAll < _.size(Game.spawns)*7) {console.log("Create Trans:",Game.spawns[i].createCreep([MOVE, MOVE, MOVE, CARRY, CARRY, CARRY], "trans"+getID(), {role: "trans", target: {}}));};
		if(countMiners>1 && countContr<3) {console.log("Create Contr:",Game.spawns[i].createCreep([MOVE, CARRY, WORK, WORK], "contr"+getID(), {role: "contr", target: contr }));};
		if(countTrans==0) {console.log("Create Trans:",Game.spawns[i].createCreep([MOVE, CARRY], "trans"+getID(), {role: "trans", target: {}}));};
	};
};
